...
## TPS62133 (Texas Instruments)
- **Function**: 3 V to 17 V synchronous step-down converter delivering up to 3 A; configured for 5 V→3.3 V here.
- **Efficiency**: ~94 % for 5 V→3.3 V at 0.5–1.0 A. Supports power-save mode for light loads; can be forced PWM via MODE/SYNC pin if desired.
- **Components**: Uses 2.2 µH inductor, 22 µF output cap typical. Soft-start programmable; PG output available for sequencing ESP32.
